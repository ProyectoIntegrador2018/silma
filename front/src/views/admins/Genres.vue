    ...
    <div align="center">
      <v-form ref="form">
        ...
        <v-layout row wrap>
          <v-dialog v-model="dialogSuccess" persistent max-width="290">
            <v-card>
              <v-card-title class="headline"
                >¡Se agrego el género!</v-card-title
              >
          ...
          <v-dialog v-model="dialogError" persistent max-width="290">
            <v-card>
              <v-card-title class="headline"
                >Error al agregar el género</v-card-title
              >
        ...
        <v-dialog v-model="dialogRepeat" persistent max-width="290">
          <v-card>
            ...
            <v-card-text
              >Ingrese un género que no exista dentro de la base de
              datos</v-card-text
            >
            <v-card-actions>
              <v-spacer></v-spacer>
              <v-btn color="red darken-1" text @click="dialogRepeat = false"
                >Entendido</v-btn
              >
            </v-card-actions>
          </v-card>
      ...
      <v-layout row wrap>
        ...
        <v-col cols="12" sm="3" v-for="genre in genres" :key="genre.name">
          <v-card color="purple">
            <v-card-text
              ><p style="color:white">{{ genre.name }}</p></v-card-text
            >
          </v-card>
        </v-col>
      </v-layout>
